    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if selectedTab == 0 {
                ...
                // updates ui on change of toggle
                    .id(reloadToggle)
        ...
        .fullScreenCover(item: $selectedJob) { job in
            ...
            // on removal of jub update ui change trigger
                .onDisappear {
                    reloadToggle.toggle()
                }
        }
    ...
    private func acceptRequest() {
        if let index = rootView.profile.jobRequests.firstIndex(where: { $0.id == job.id }) {
            ...
            if let currentValue = rootView.profile.currentAthletes[athlete] {
                ...
                let updatedTotal = currentValue.1 + Double(acceptedJob.cost)
                rootView.profile.currentAthletes[athlete] = (updatedSessionCount, updatedTotal)
            } else {
                rootView.profile.currentAthletes[athlete] = (1, Double(acceptedJob.cost))
            }
