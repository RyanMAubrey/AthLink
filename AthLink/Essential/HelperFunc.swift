...
let log = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.Athlinktest", category: "AthLink")

// MARK: Enums
// ** Sports types **
enum Sports: String, Hashable, Codable, CustomStringConvertible, CaseIterable {
...
// ** Session Types **
enum GroupType: String, Hashable, Codable, CustomStringConvertible, CaseIterable {
...
func athLinkWholeDollar(_ value: Double) -> String {
    "$\(Int(value.rounded()))"
}

func athLinkDateWithDayAndYear(_ date: Date) -> String {
    let formatter = DateFormatter()
    formatter.dateFormat = "EEEE, MMMM d, yyyy"
    return formatter.string(from: date)
}

func athLinkSessionDateTime(_ date: Date) -> String {
    ...
    let formatter = DateFormatter()
    formatter.dateFormat = "h:mm a 'on' EEEE, MMMM"
    return "\(formatter.string(from: date)) \(day)\(suffix)"
}

func athLinkRelativeTime(_ date: Date) -> String {
...
struct CoachProfilePatch: Encodable {
    ...
    let coach_past_sessions: [Session]?
...
func coordinatesFromZip(_ zip: String) async -> CLLocationCoordinate2D? {
    do {
        ...
        log.error("Forward geocode failed: \(error.localizedDescription)")
        return nil
    }
}
...
func getReview(client: SupabaseClient, coachID: UUID, reviewerID: UUID) async -> Review? {
    do {
        ...
        log.error("Failed to get review: \(error.localizedDescription)")
        return nil
    }
}

// ** Gets all reveiws for a coach **
func getCoachReviews(client: SupabaseClient, coachID: UUID) async -> [Review] {
    do {
        let reviews: [Review] = try await client
            .from("reviews")
            .select()
        ...
        log.error("Failed to get coach reviews: \(error.localizedDescription)")
        return []
    }
}
...
func submitReview(client: SupabaseClient, coachID: UUID, reviewerID: UUID, star: Float, quote: String) async -> Bool {
    do {
        ...
        log.error("Failed to submit review: \(error.localizedDescription)")
        return false
    }
}
...
func updateReview(client: SupabaseClient, reviewID: UUID, star: Float, quote: String) async -> Bool {
    do {
        ...
        log.error("Failed to update review: \(error.localizedDescription)")
        return false
    }
}
...
func getCoachRating(client: SupabaseClient, coachID: UUID) async -> CoachRating? {
    do {
        ...
        log.error("Failed to get coach rating: \(error.localizedDescription)")
        return nil
    }
}
...
// Notificaitons helper
func sendPushNotification(client: SupabaseClient, userId: UUID, title: String, body: String) async {
    do {
        let url = URL(string: "\(infoValue(key: "SUPABASE_URL"))/functions/v1/send-push-notifications")!
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.setValue("Bearer \(client.auth.currentSession?.accessToken ?? "")", forHTTPHeaderField: "Authorization")
        ...
        log.error("Push notification failed: \(error.localizedDescription)")
    }
}
