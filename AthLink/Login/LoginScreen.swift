                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            HStack(spacing: 10) {
                                Image(systemName: "gift")
                                    ...
                                    .frame(width: 20)
                                TextField("Enter code", text: $referralInput)
                            ...
                            .overlay(
                                RoundedRectangle(cornerRadius: 10)
                                    .stroke(referralInput.isEmpty ? Color.clear : Color.blue.opacity(0.4), lineWidth: 1.5)
                            )
                        }
                    }
                    .padding(20)
                        ...
                        Task {
                            ...
                            do {
                                ...
                                _ = try await rootView.client
                                    ...
                                    .execute()
                                // Set up stripe
                                let stripeURL = URL(string: "\(infoValue(key: "SUPABASE_URL"))/functions/v1/smart-responder")!
                                var stripeReq = URLRequest(url: stripeURL)
                                stripeReq.httpMethod = "POST"
                                stripeReq.setValue("application/json", forHTTPHeaderField: "Content-Type")
                                stripeReq.setValue("Bearer \(res.session?.accessToken ?? "")", forHTTPHeaderField: "Authorization")
                                stripeReq.httpBody = try JSONEncoder().encode([
                                    "name": "\(firstName) \(lastName)",
                                    "user_id": res.user.id.uuidString
                                ])
                                let (stripeData, _) = try await URLSession.shared.data(for: stripeReq)
                                if let stripeJSON = try? JSONDecoder().decode([String: String].self, from: stripeData),
                                   let customerId = stripeJSON["customerId"] {
                                    // Update table
                                    try await rootView.client
                                        .from("profiles")
                                        .update(["stripe_customer_id": customerId])
                                        .eq("id", value: res.user.id)
                                        .execute()
                                }
                                // Referrals
                                let trimmedReferral = referralInput.trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
                                if !trimmedReferral.isEmpty {
                                    try await rootView.client.rpc("apply_referral", params: [
                                        "p_user_id": res.user.id.uuidString,
                                        "p_code": trimmedReferral
                                    ]).execute()
                                }
