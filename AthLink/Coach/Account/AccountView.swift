...
struct AccountView: View {
    ...
    // Payment info — derived from Stripe customer ID
    ...
    @State private var checkrLoading = false
    ...
    private func updateTotal(type: Bool) {
        let totalDouble = Double(type ? idollars : gdollars)
        if type {
    ...
    private func saveChanges() {
        ...
        Task {
            do {
                ...
                log.error("Coach profile update failed: \(error.localizedDescription)")
            }
        }
    }

    private func centerMapOnCurrentLocation() {
        if let selectedTrainingLoc {
            camera = .region(MKCoordinateRegion(
                center: selectedTrainingLoc.coordinate,
                span: MKCoordinateSpan(latitudeDelta: 0.02, longitudeDelta: 0.02)
            ))
        } else if let region = regionThatFits(trainingLoc) {
            camera = .region(region)
        } else if let cl = rootView.currentLocation {
            camera = .region(MKCoordinateRegion(
    ...
    private func startOnboarding() async {
        guard let connectId = rootView.profile.stripeConnectId else { return }
        onboardingLoading = true
        defer { Task { @MainActor in onboardingLoading = false } }
        do {
            ...
            log.info("Onboarding response received")
            ...
            log.error("Onboarding link failed: \(error.localizedDescription)")
        }
    }
    ...
    var body: some View {
                ...
                VStack(spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        Text("Payouts")
                            .font(.headline)
                        HStack(spacing: 12) {
                            Image(systemName: rootView.profile.completedOnboarding ? "banknote.fill" : "banknote")
                                .font(.title2)
                                .foregroundColor(rootView.profile.completedOnboarding ? .green : .orange)
                            VStack(alignment: .leading, spacing: 2) {
                                Text(rootView.profile.completedOnboarding ? "Payouts Enabled" : "Payouts Not Set Up")
                                    .font(.subheadline)
                                    .fontWeight(.medium)
                                Text(rootView.profile.completedOnboarding ? "Bank account connected via Stripe" : "Link your bank account to receive payments")
                                    .font(.caption)
                                    .foregroundColor(.gray)
                            }
                            Spacer()
                            if !rootView.profile.completedOnboarding {
                                Button(action: {
                                ...
                                .disabled(onboardingLoading || rootView.profile.stripeConnectId == nil)
                            } else {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        Text("Background Check")
                            .font(.headline)
                        HStack(spacing: 12) {
                            Image(systemName: rootView.profile.completedCheckr ? "shield.checkmark.fill" : "shield")
                                .font(.title2)
                                .foregroundColor(rootView.profile.completedCheckr ? .green : .orange)
                            VStack(alignment: .leading, spacing: 2) {
                                Text(rootView.profile.completedCheckr ? "Background Check Passed" : "Background Check Not Set Up")
                                    .font(.subheadline)
                                    .fontWeight(.medium)
                                Text(rootView.profile.completedCheckr ? "Verified via Checkr" : "Complete a background screening for athlete safety")
                                    .font(.caption)
                                    .foregroundColor(.gray)
                            }
                            Spacer()
                            if !rootView.profile.completedCheckr {
                                Button(action: {
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        HStack(alignment: .center, spacing: 16) {
                            ...
                            Picker(selection: $idollars, label: Text("Rate")) {
                                ForEach(Array(stride(from: 0, through: 200, by: 5)), id: \.self) { value in
                        ...
                        HStack(alignment: .center, spacing: 16) {
                            ...
                            Picker(selection: $gdollars, label: Text("Rate")) {
                                ForEach(Array(stride(from: 0, through: 200, by: 5)), id: \.self) { value in
                                    Text(value == 0 ? "Off" : "$\(value)")
                                }
                            }
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Cancellation Notice")
                            .font(.headline)
                        Text("How many hours in advance athletes must cancel to avoid being charged.")
                            .font(.caption)
                            .foregroundColor(.secondary)
                        HStack {
                            Text("Hours:")
        ...
        .onAppear() {
            ...
            idollars = Int((iCost ?? 0) / 5.0) * 5
            gdollars = Int((gCost ?? 0) / 5.0) * 5
            itotal = iCost
            gtotal = gCost
            // Image
            if rootView.profile.imageURL.hasPrefix("http"),
        ...
        .alert("Save Changes?", isPresented: $showSaveAlert) {
            ...
            Text("Save your coach profile changes?")
        }
