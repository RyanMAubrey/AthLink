...
struct RequestSess: View {
    ...
    private var isEditingPost: Bool {
        !isDirectRequest && !chatPage && rootView.profile.postedSession != nil
    }
    
    // Chck last page
    private var chatPage: Bool { rootView.lastPage == "Chat" }
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 4) {
                    ...
                    if let coach = targetCoach {
                        ...
                        Text(isEditingPost ? "Edit Your Session" : chatPage ? "Send a Session to this person" : "Post a Session")
                            .font(.title3)
                            .fontWeight(.bold)
                        Text(chatPage ? "The session will be sent to the chat" : "Coaches in your area will see this posting")
                            .font(.subheadline)
                            .foregroundColor(.gray)
                    }
                }
                ...
                VStack(spacing: 16) {
                    ...
                    // Hourly rate — posting and chat mode
                    if !isDirectRequest || chatPage {
                        divider
    ...
    private func prefill() {
        if chatPage, let existing = rootView.selectedJobSession {
            ...
            rootView.selectedJobSession = nil
            return
        }
        if isDirectRequest || chatPage {
            setDefaultLocation()
    ...
    private func sendDirectRequest() async {
        ...
        do {
            ...
            await sendPushNotification(
                ...
                body: "\(rootView.profile.fullName) sent you a session request"
            )

            rootView.path.removeLast()
        } catch {
            log.error("Failed to send session request: \(error.localizedDescription)")
        }
    }

    private func sendChatSession() async {
        guard let loc = location, let partnerId = rootView.chatPartner?.id else { return }
        let rate = Double(typeRate) ?? 0
        isSaving = true
        defer { isSaving = false }
        ...
        let isCoach = rootView.rootView == .Coach
        let msg = MessageRow(
        ...
        do {
            ...
            log.error("Failed to send chat session: \(error.localizedDescription)")
        }
    }
    ...
    private func submitOrUpdatePost() async {
        ...
        do {
            ...
            log.error("Failed to upsert posted session: \(error.localizedDescription)")
        }
    }
    ...
    private func deletePost() async {
        do {
            ...
            log.error("Failed to delete posted session: \(error.localizedDescription)")
        }
    }
}
